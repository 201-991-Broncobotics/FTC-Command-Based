...
public class PIDCalculator {
    ...
    private final double kP, kI, kD, minPosition, maxPosition, minPower, maxPower, acceleration, calibration_time, tolerance;
    private final DoubleSupplier positionSup;

    private double integral, previous_error, previous_time, target_position, last_time, last_manually_targeted_position, previous_power;
    private boolean enable_limiting = true; // if we want to be bounded by min/max position. Otherwise we only do PI

    public PIDCalculator(double kP, double kI, double kD, double minPosition, double maxPosition,
    ...
    private double getCorrection() {
        ...
        if (Math.abs(p) < maxPower) {
            if (p != 0) p *= Math.pow(Math.abs(p) / maxPower, kD - 1);
        ...
        double d = (error - previous_error) / delta_time * kD;
